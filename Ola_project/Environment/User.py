...
class User0(User):
    def __init__(self, primary):
        ...
        self.reservation_price = [15,25,35,45,55] + np.random.normal(1, scale=2, size=5) # average reservation price
...
class User1(User):
    def __init__(self, primary):
        User.__init__(self, primary)
        self.reservation_price = [15,25,35,45,55] + np.random.normal(1, scale=4, size=5) # more variable reservation price
...
class User2(User):
    def __init__(self, primary):
        User.__init__(self, primary)
        self.reservation_price = 5 + [15,25,35,45,55] + np.random.normal(1, scale=3, size=5) # higher reservation price
        self.P = User.generate_graph(self, np.random.uniform(0, 1, size=(5,5))) # more variable influence probabilities


#omogenous user class with fixed graph weights (for the first steps)     
class FixedUsers(User):
    def __init__(self, primary):
        User.__init__(self, primary)
        self.reservation_price = [15,25,35,45,55] + np.random.normal(0, scale=2.5, size=5) # average reservation price
       ...
       [0.04359364, 0.45397953, 0.31312689, 0.41816953, 0.24363287]])) #fixed influence probabilities (graph weights)
