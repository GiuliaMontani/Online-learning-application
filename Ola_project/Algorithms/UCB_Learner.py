...
class UCB(Learner):
    ...
    def pull_arm(self):
        ...
        #and we store and use it to compare the upper confidence
        if self.t ==2*self.n_arms:
            for i in range(5):
                self.make_comparable[i] = np.max(self.expected_rewards[i]) 

        idx = np.zeros(5)
        upper_conf = np.zeros((5,4))

        #chose the arm with the highest expected_rewards for each product, 
        # taking in to account the confidence interval (which gets smaller more time the corresponding arm is pulled) 
        for i in range(5):
            #confidence is multiplied by np.max(self.expected_rewards[i]) to make the upper confidence bound comparable (some have higher expected rewards) 
            upper_conf[i] = self.expected_rewards[i] + self.confidence[i] * self.make_comparable[i]
            #increasing the term make_comparable we get a longer exploration phase but we have more accuracy in choosing the best candidates
