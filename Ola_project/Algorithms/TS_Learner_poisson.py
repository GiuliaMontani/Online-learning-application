...
class TS_poisson(Learner):
    def __init__(self, n_arms):
        ...
        self.lambda_poisson = np.array([np.zeros(4)]*5) #mean quantities bought per product (in step 4 they are variable)
    ...
    def pull_arm(self, margins_matrix):
        ...
        #and we consider the units bought distributed as a poisson with lambda = mean units bought per arm until now
        for i in range(5):
