...
class TS(Learner):
    def __init__(self, n_arms):
        ...
        #self.mean_quantities = np.array([np.zeros(4)]*5) #mean quantities bought per product (in step 3 they are fixed)
    ...
    def pull_arm(self, margins_matrix):
        ...
        for i in range(5):
            idx[i] = np.argmax(np.random.beta(self.beta_parameters[i][:, 0], self.beta_parameters[i][:, 1]) * margins_matrix[i,:])#*self.mean_quantities[i]
    ...
    def update(self, pulled_arm, reward, clicks, purchases, daily_units):
        ...
        for i in range(5):
            ...
            #if purchases[i]!=0:
                #self.mean_quantities[i][int(pulled_arm[i])] = (self.mean_quantities[i][int(pulled_arm[i])]* (self.counter_per_arm[i][int(pulled_arm[i])]-1) + daily_units[i]/purchases[i])/ self.counter_per_arm[i][int(pulled_arm[i])] if self.mean_quantities[i][int(pulled_arm[i])] > 0 else daily_units[i]/purchases[i]
