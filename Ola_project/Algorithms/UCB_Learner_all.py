...
class UCB_all(Learner_all):
    ...
    def update(self, pulled_arm, reward):
        self.t += 1
        for i in range(5):
            self.expected_rewards[i][pulled_arm[i]] = (self.expected_rewards[i][pulled_arm[i]]*(self.t-1)  + reward[i]) / self.t
            n_samples = np.size(self.rewards_per_arm[i][pulled_arm[i]])
            self.confidence[i][pulled_arm[i]] = (2 * np.log(self.t) / n_samples) ** 0.5 if n_samples > 0 else np.inf
